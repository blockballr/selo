...
/// Represents an SPL token transfer operation.
#[derive(Debug, Clone)]
...
impl TokenTransfer {
    pub fn resolve(
        ...
        let dest_owner = crate::address::decode_pubkey(destination)?;
        let mint_pubkey = crate::address::decode_pubkey(mint)?;
        let token_program = crate::address::decode_pubkey(TOKEN_PROGRAM_STR)?;

        let source_ata = crate::pda::associated_token_address(owner, &mint_pubkey, &token_program)?;
        let destination_ata =
            crate::pda::associated_token_address(&dest_owner, &mint_pubkey, &token_program)?;
    ...
    pub fn build_message(&self, blockhash_b58: &str) -> Result<Vec<u8>, String> {
        ...
        crate::message::compile_message(
            &self.owner,
            &[crate::message::Instruction {
                program_id: token_program,
                accounts: vec![
                    crate::message::AccountMeta::writable(self.source_ata),
                    crate::message::AccountMeta::readonly(self.mint),
                    crate::message::AccountMeta::writable(self.destination_ata),
                    crate::message::AccountMeta::signer_writable(self.owner),
                ],
                data,
            }],
            blockhash_b58,
